...
public class CrudOrganizersController implements Initializable {
    ...
    private final TableView<Organizer> table = createTable();
    private List<Organizer> data;
    private int rowsPerPage = 10;
    private List<Integer> variable = new ArrayList<>();


    private TableView<Organizer> createTable() {
        TableView<Organizer> table = new TableView<>();

        TableColumn<Organizer, Integer> idColumn = new TableColumn<>("Id");
        ...
        TableColumn<Organizer, String> surnameColumn = new TableColumn<>("Surname");
        surnameColumn.setCellValueFactory(param -> param.getValue().surname);
        surnameColumn.setPrefWidth(80);

        TableColumn<Organizer, String> formattedDateColumn = new TableColumn<>("birthday");
    ...
    private List<Organizer> createData() throws UnirestException, IOException {
        List<ArtistResponseDTO> list = Arrays.asList(new GsonBuilder().create().fromJson(
                Unirest.post("http://localhost:8080/organizer/fetch/list")
                        .header("content-type", "application/json")
                        .body("{ }")
                        .asJson().getBody().toString(), ArtistResponseDTO[].class)
        );

        return list.stream().map(artist -> new Organizer(artist.getId(), artist.getName(), artist.getSurname(), artist.getFormattedDate()))
    ...
    void delete(ActionEvent event) throws UnirestException, IOException {
        HttpResponse<String> deleteResponse = Unirest.post("http://localhost:8080/organizer/delete/{id}")
    ...
    void create(ActionEvent event) throws UnirestException, IOException {
        ...
        HttpResponse<JsonNode> addResponse = Unirest.post("http://localhost:8080/organizer/add")
                .header("content-type", "application/json")
                .body(artistDTO)
                .asJson();
    ...
    void update(ActionEvent event) throws UnirestException, IOException {
        ...
        HttpResponse<JsonNode> addResponse = Unirest.post("http://localhost:8080/organizer/update")
                .header("content-type", "application/json")
                .body(artistDTO)
                .asJson();
        System.out.println("ADD status = " + addResponse.getStatus());
    ...
    public static class Organizer {
        ...
        private Organizer(Integer id, String name, String surname, String formattedDate) {
