...
public class CrudArtistsController implements Initializable {
    ...
    private final TableView<Artist> table = createTable();
    private List<Artist> data;
    private int rowsPerPage = 10;
    private List<Integer> variable = new ArrayList<>();


    private TableView<Artist> createTable() {
        TableView<Artist> table = new TableView<>();

        TableColumn<Artist, Integer> idColumn = new TableColumn<>("Id");
        ...
        TableColumn<Artist, String> surnameColumn = new TableColumn<>("Surname");
        surnameColumn.setCellValueFactory(param -> param.getValue().surname);
        surnameColumn.setPrefWidth(80);

        TableColumn<Artist, String> formattedDateColumn = new TableColumn<>("birthday");
    ...
    private List<Artist> createData() throws UnirestException, IOException {
        List<ArtistResponseDTO> list = Arrays.asList(new GsonBuilder().create().fromJson(
                Unirest.post("http://localhost:8080/artist/fetch/list")
                        .header("content-type", "application/json")
                        .body("{ }")
                        .asJson().getBody().toString(), ArtistResponseDTO[].class)
        );

        return list.stream().map(artist -> new Artist(artist.getId(), artist.getName(), artist.getSurname(), artist.getFormattedDate()))
    ...
    void delete(ActionEvent event) throws UnirestException, IOException {
        HttpResponse<String> deleteResponse = Unirest.post("http://localhost:8080/artist/delete/{id}")
    ...
    void create(ActionEvent event) throws UnirestException, IOException {
        ...
        HttpResponse<JsonNode> addResponse = Unirest.post("http://localhost:8080/artist/add")
                .header("content-type", "application/json")
                .body(artistDTO)
                .asJson();
    ...
    void update(ActionEvent event) throws UnirestException, IOException {
        ...
        HttpResponse<JsonNode> addResponse = Unirest.post("http://localhost:8080/artist/update")
                .header("content-type", "application/json")
                .body(artistDTO)
                .asJson();
        System.out.println("ADD status = " + addResponse.getStatus());
    ...
    void edit(ActionEvent event) {
        Artist selected = table.getSelectionModel().getSelectedItem();
    ...
    public static class Artist {
        ...
        private Artist(Integer id, String name, String surname, String formattedDate) {
